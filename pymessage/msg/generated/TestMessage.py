...
class TestMessage( MessageBase ):
    ...
    def toString(self, _indent: int = 0 ) -> str:
        ...
        _buffer += self._blanks( _indent ) + "intValue : " + str( self.intValue) + "\n"
        _buffer += self._blanks( _indent ) + "strValue : " + str( self.strValue) + "\n"
        _buffer += self._blanks( _indent ) + "stringArray : " + str( self.stringArray) + "\n"
        _buffer += self._blanks( _indent ) + "intArray : " + str( self.intArray) + "\n"
        if self.subMsgs is None:
           _buffer += self._blanks( _indent ) + "subMsgs : None \n"
        else:
                    
           _buffer += self._blanks( _indent ) + "subMsgs : \n"
           _idx = 0
           for _m in self.subMsgs:
                _idx += 1;
                _buffer += self._blanks( _indent + 2 ) + "[" + str(_idx) +"] \n"
                _buffer += _m.toString( _indent + 4) + "\n"
        _buffer += self._blanks( _indent ) + "endValue : " + str( self.endValue) + "\n"
        return _buffer
